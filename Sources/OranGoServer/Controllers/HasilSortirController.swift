...
struct HasilSortirController: RouteCollection {
    func boot(routes: any RoutesBuilder) throws {
        ...
        group.get(use: index)
        group.post(use: create)   // Mac kirim ke sini, dilindungi API key
    }
    ...
    func create(req: Request) async throws -> HasilSortir {
        ...
        let hasil = HasilSortir(
            batchID: input.batchId, gradeID: try grade.requireID(), retailGradeID: input.retailGradeId,
            waktuScan: Date(), diameter: input.diameter, berat: input.berat,
            warnaOranye: input.warnaOranye, bentukWajar: input.bentukWajar
        )
